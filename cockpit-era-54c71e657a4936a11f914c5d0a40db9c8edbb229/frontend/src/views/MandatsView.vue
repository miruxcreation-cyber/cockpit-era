...
const confirmDeleteTarget = ref<Mandat | null>(null)
...
function calcNetSamir(prix: string, taux: string, source: string) {
  ...
  return Math.round(apresMA * 0.32 * 0.78).toLocaleString('fr-FR') + ' €'
}
...
function calcNet(m: Mandat) {
  ...
  return { com: Math.round(com), net: Math.round(apresMA * 0.32 * 0.78) }
}
...
const actifs = computed(() =>
  store.mandats.filter(m => ['actif','visite','offre','compromis','acte','signature'].includes(m.statut) && matchPipe(m) && matchQuery(m))
)
...
function mandateType(m: Mandat) {
  const t: Record<string, string> = { exclusif: '⭐ Exclusif', simple: '📄 Simple', 'co-exclusif': '👥 Co-exclusif' }
  return t[m.mandat || ''] || m.mandat || '—'
}
...
function openDeleteConfirm(m: Mandat) {
  confirmDeleteTarget.value = m
...
function confirmDelete() {
  ...
  store.deleteMandat(confirmDeleteTarget.value.id)
...
<template>
  <section class="panel on mandats-view">
      ...
      <div v-for="item in actifs" :key="item.id" class="card-outer">
        ...
        <article class="card" :class="{ swiped: swipedId === item.id }" @click="toggleSwipe(item.id)">
          ...
          <template v-if="item.prix">
            <div class="card-row"><span class="card-lbl">🏷️ Prix FAI</span><span class="card-val" style="font-weight:700">{{ fmtP(item.fai || Math.round(Number(item.prix)*(1+(item.taux||7)/100))) }}</span></div>
          ...
          <div class="card-ft">
            <div style="display:flex;gap:5px;flex-wrap:wrap;align-items:center">
              <span class="badge" :class="statutBadgeClass[item.statut]||'b-gray'" style="cursor:pointer" @click.stop="openSheet(item)">{{ statutLabels[item.statut]||item.statut }} 〉</span>
        ...
        <div class="modal-sheet">
          ...
          <div class="modal-body">
            ...
            <div>
              ...
              <textarea v-model="formMandat.notes" class="field-input field-textarea" placeholder="ex. 3e étage, balcon 6m², parking…" />
            </div>
          </div>
    ...
    <Teleport to="body">
      <div v-if="confirmDeleteOpen" class="modal-overlay" @click.self="confirmDeleteOpen=false">
        <div class="confirm-box">
          <div style="font-size:40px;margin-bottom:14px">🗑️</div>
          <div class="confirm-title">Supprimer ce mandat ?</div>
          <div class="confirm-adresse">{{ confirmDeleteTarget?.vendeur }} — {{ confirmDeleteTarget?.adresse?.split(',')[0] }}</div>
          <div style="font-size:12px;color:var(--red);margin-top:6px">Cette action est irréversible.</div>
          <div class="confirm-actions">
            <button class="btn" style="flex:1;background:var(--surface2);color:var(--text2);border:0.5px solid var(--border)" @click="confirmDeleteOpen=false">Annuler</button>
            <button class="btn" style="flex:1;background:var(--red);color:#fff;border:none" @click="confirmDelete">Supprimer</button>
          </div>
        </div>
      </div>
    </Teleport>
    ...
    <div class="status-sheet" :class="{ open: sheetOpen }">
      ...
      <div class="sheet-title">{{ sheetTarget?.adresse?.split(',')[0] }}</div>
      <div class="sheet-options">
        ...
        <button class="sheet-opt" style="color:var(--red);margin-top:4px" @click="openDeleteConfirm(sheetTarget!)">
          🗑️ Supprimer ce mandat
        </button>
      </div>
    ...
    <div class="status-sheet" :class="{ open: matchSheetOpen }" style="max-height:70vh;overflow-y:auto">
      ...
      <div v-if="matchTarget" style="padding:0 4px">
        <div v-if="!acqMatches(matchTarget).length" style="text-align:center;padding:24px;color:var(--text3);font-size:13px">Aucun acquéreur compatible</div>
...
/* Modals */
.modal-overlay { position: fixed; inset: 0; background: rgba(0,0,0,0.65); z-index: 300; display: flex; align-items: flex-end; justify-content: center; }
...
/* Confirm */
.confirm-box { background: var(--surface); border-radius: 20px; width: min(320px, 88vw); padding: 28px 20px 20px; text-align: center; align-self: center; }
.confirm-title { font-size: 17px; font-weight: 700; margin-bottom: 6px; }
.confirm-adresse { font-size: 13px; color: var(--text3); }
